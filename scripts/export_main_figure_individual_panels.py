...
def export_figure2_panels() -> None:
    ...
    if not network.empty:
        ...
        for row in network.itertuples(index=False):
            ...
            ax.annotate(
                row.state_category,
                (row.network_x, row.network_y),
                xytext=(8 if row.network_x >= 0 else -8, 0),
    ...
    ax.margins(x=0.30, y=0.25)
    ax.set_title("Response-linked single-cell state network", pad=12)
    ax.set_aspect("equal")
    ax.set_axis_off()
...
def export_figure3_panels() -> None:
    ...
    perf = read_panel("figure_3_panelb_sctime_reliability_matrix_across_validation_designs.tsv")
    ...
    coef = read_panel("figure_3_panelh_interpretable_sctime_coefficients.tsv")
    ...
    ax.text(0.02, 0.98, f"Highlighted: scTIME >= {FIGURE3A_RESEARCH_VALUE_THRESHOLD:.2f}", transform=ax.transAxes, va="top", ha="left", fontsize=6, color=PALETTE["dark"])
    fig.colorbar(sc, ax=ax, fraction=0.046, pad=0.03, label="scTIME score")
    ...
    ax.set_title("scTIME reliability across evaluation designs")
    ax.set_xlabel("")
    ax.set_ylabel("Metric")
    ax.tick_params(axis="x", rotation=25)
    save_panel(fig, 3, "B", "scTIME reliability matrix across evaluation designs")
    ...
    fig, ax = new_fig(4.0, 3.4)
    sns.lineplot(data=dca, x="threshold", y="net_benefit_model", ax=ax, label="scTIME", color=PALETTE["positive"], linewidth=1.6)
    ...
    lollipop(ax, coef_plot["label"], coef_plot["coefficient"], "Interpretable scTIME coefficients", "ElasticNet coefficient")
    save_panel(fig, 3, "H", "Interpretable scTIME coefficients")
    ...
    ax.set_xlabel("Labeled bulk patients ordered by response and scTIME score")
    ax.set_ylabel("")
...
def export_figure5_panels() -> None:
    ...
    f5d = read_panel("figure_5_paneld_tcga_exclusion_index_density_vs_sctime.tsv")
    ...
    cohort_colors = category_palette(f5b["dataset"])
    cohort_stats = []
    for dataset, cohort in f5b.groupby("dataset", observed=True):
        sub = cohort[["Myeloid/macrophage", "SPP1_macrophage"]].dropna()
        color = cohort_colors[str(dataset)]
        ax.scatter(sub["Myeloid/macrophage"], sub["SPP1_macrophage"], s=22, alpha=0.58, color=color, label=clean_label(dataset))
        if len(sub) >= 4:
            rho, p = stats.spearmanr(sub["Myeloid/macrophage"], sub["SPP1_macrophage"])
            sns.regplot(data=sub, x="Myeloid/macrophage", y="SPP1_macrophage", scatter=False, ci=None, line_kws={"color": color, "linewidth": 1.2}, ax=ax)
            cohort_stats.append(f"{clean_label(dataset)}: rho={rho:.2f}, P={p:.2g}")
    ax.text(0.03, 0.97, "\n".join(cohort_stats), transform=ax.transAxes, va="top", fontsize=6)
    ax.legend(frameon=False, fontsize=5.5, loc="lower right")
    ax.set_title("Cohort-stratified myeloid-SPP1 consistency")
    ...
    f5c["cohort"] = f5c["analysis"].str.replace(r"^bulk NNLS ", "", regex=True).map(clean_label)
    grouped_dot_matrix(ax, f5c, "x", "module", "cohort", "rho", "p_value", "Cohort-stratified deconvolution-module correlations", "Spearman rho", "Cohort", legend_outside_top=True)
    save_panel(fig, 5, "C", "Deconvolution-module correlation matrix")

    fig, ax = new_fig(4.2, 3.5)
    project_colors = category_palette(f5d["project"])
    project_stats = []
    for project, cohort in f5d.groupby("project", observed=True):
        color = project_colors[str(project)]
        ax.scatter(cohort["immune_exclusion_index"], cohort["scTIME_AI_score"], s=9, alpha=0.18, color=color, linewidth=0, label=project)
        rho, p = stats.spearmanr(cohort["immune_exclusion_index"], cohort["scTIME_AI_score"])
        sns.regplot(data=cohort, x="immune_exclusion_index", y="scTIME_AI_score", scatter=False, ci=None, line_kws={"color": color, "linewidth": 1.4}, ax=ax)
        project_stats.append(f"{project}: rho={rho:.2f}, P={p:.2g}")
    ax.text(0.03, 0.97, "\n".join(project_stats), transform=ax.transAxes, va="top", fontsize=6)
    ax.legend(frameon=False, fontsize=6, loc="lower right")
    ax.set_title("TCGA project-stratified exclusion context")
    ax.set_xlabel("Exclusion index")
    ax.set_ylabel("scTIME score")
    save_panel(fig, 5, "D", "TCGA exclusion-index density vs scTIME")
    ...
    plot["project"] = plot["analysis"].str.replace(r"^TCGA exclusion ", "", regex=True)
    grouped_dot_matrix(ax, plot, "program", "target", "project", "rho", "p_value", "Project-stratified TCGA exclusion co-programs", "Spearman rho", "Project", legend_outside_top=True)
    save_panel(fig, 5, "F", "TCGA exclusion co-program structure")


def export_figure6_panels() -> None:
    f6a = read_panel("figure_6_panela_cd274_and_held_out_sctime_scores_in_ici_cohorts.tsv")
    f6b = read_panel("figure_6_panelb_matched_within_cohort_sctime_oof_and_cd274_response_stratification.tsv")
    f6c = read_panel("figure_6_panelc_cd274_high_low_oof_sctime_feature_contrast.tsv")
    ...
    sns.scatterplot(data=f6a, x="CD274_expr", y="scTIME_oof_score", hue="response", style="dataset", markers=marker_map, s=54, palette=response_palette("no benefit", "benefit/MPR"), ax=ax)
    rho, p = stats.spearmanr(f6a["CD274_expr"], f6a["scTIME_oof_score"])
    ax.text(0.05, 0.95, f"rho={rho:.2f}\nP={p:.2g}\nheld-out score", transform=ax.transAxes, va="top", fontsize=7)
    ax.set_title("CD274 and held-out scTIME scores")
    ax.set_xlabel("CD274 expression/module")
    ax.set_ylabel("scTIME OOF probability")
    ax.legend(frameon=False, fontsize=6)
    save_panel(fig, 6, "A", "CD274 and held-out scTIME scores in ICI cohorts")
    ...
    colors = [
        ...
        PALETTE["negative"], PALETTE["neutral"], PALETTE["positive"],
        PALETTE["negative"], PALETTE["positive"], PALETTE["negative"], PALETTE["positive"],
    ][: len(resp_plot)]
    ...
    ax.set_xlim(0, 1.05)
    ax.set_title("Matched within-cohort scTIME OOF and CD274 splits")
    ax.set_xlabel("Benefit/MPR fraction with 95% Wilson CI")
    ax.set_ylabel("")
    save_panel(fig, 6, "B", "Matched within-cohort scTIME OOF and CD274 response stratification")

    fig, ax = new_fig(4.5, 3.4)
    f6c["label"] = f6c["feature"].map(lambda x: FEATURE_LABELS.get(x, x))
    lollipop(ax, f6c["label"], f6c["difference"], "CD274-high / low OOF-scTIME feature contrast", "High OOF-scTIME minus CD274-high / low OOF-scTIME")
    save_panel(fig, 6, "C", "CD274-high low-OOF-scTIME feature contrast")
...
def export_figure7_panels() -> None:
    f7a = read_panel("figure_7_panela_gse135222_exploratory_pfs_association.tsv")
    f7b = read_panel("figure_7_panelb_survival_association_summary.tsv")
    f7c = read_panel("figure_7_panelc_tcga_luad_lusc_sctime_distributions.tsv")
    ...
    ax.set_title("GSE135222 exploratory PFS association")
    ax.set_xlabel("PFS time")
    ax.set_ylabel("PFS probability")
    ax.legend(frameon=False)
    save_panel(fig, 7, "A", "GSE135222 exploratory PFS association")
    ...
    ax.set_xlabel("Hazard ratio per scTIME score")
    ax.set_ylabel("")
    ...
    ax.set_title("TCGA-LUAD/LUSC scTIME distributions")
    ax.set_xlabel("")
    ax.set_ylabel("scTIME score")
    save_panel(fig, 7, "C", "TCGA-LUAD LUSC scTIME distributions")

    fig, ax = new_fig(4.6, 3.4)
    f7d = f7d.copy()
    f7d["cohort_omics"] = f7d["cohort_omics"].str.replace("CPTAC-", "", regex=False)
    ...
    ax.set_xlabel("")
    ax.set_ylabel("scTIME score")
    ax.tick_params(axis="x", rotation=0, labelsize=7, pad=2)
    strip_duplicate_legend(ax)
