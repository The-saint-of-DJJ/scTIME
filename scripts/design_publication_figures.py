...
MAIN_FIGURE_PLAN = [
    {
        "figure": "Figure 1",
        "file": "user-drawn framework figure; no generated image file",
        "purpose": "Graphical study framework linking public cohorts, single-cell discovery, scTIME-AI modeling, mechanistic axes and external validation.",
        "panels": [
            "A Public multi-omics cohort map: GSE207422/GSE243013 single-cell discovery, GSE126044/GSE207422 bulk response modeling, GSE135222 survival validation, TCGA and CPTAC external validation",
            "B Analytical flow: marker-program scoring, cell-state projection, ElasticNet scTIME-AI training, cross-cohort validation, interpretation and biology validation",
            "C Biological hypothesis: responder activation/CXCL10-CXCR3 recruitment axis versus myeloid/TGF-beta/hypoxia exclusion axis",
            "D Translational output: interpretable patient-level probability, feature contribution profile and validation endpoints",
        ],
    },
    {
        "figure": "Figure 2",
    ...
    },
    {
        ...
        "purpose": "Complex scTIME-AI reliability, cross-cohort validation and interpretability figure.",
        "panels": [
            "A Bulk immune-state PCA with response and score overlay",
            "B scTIME-AI reliability matrix across validation designs",
            ...
            "H Interpretable scTIME-AI coefficients",
    ...
    },
    {
        ...
        "panels": [
            ...
            "B Myeloid deconvolution moderately aligns with SPP1 macrophage score",
            "C Deconvolution-module correlation matrix",
            "D TCGA exclusion-index density vs scTIME-AI",
    ...
    },
    {
        ...
        "purpose": "PD-L1/scTIME-AI complementarity and TCR ecology without low-density single-variable TCR panels.",
        "panels": [
            "A CD274 and scTIME-AI co-state in ICI cohorts",
            "B High-confidence scTIME-AI enrichment versus CD274 stratification",
            "C CD274-high/scTIME-AI-low immune-feature contrast",
    ...
    },
    {
        ...
        "purpose": "External survival and cross-omics validation kept compact and non-redundant.",
        "panels": [
            "A GSE135222 PFS Kaplan-Meier validation",
            "B Survival association summary",
            "C TCGA-LUAD/LUSC scTIME-AI distributions",
    ...
    },
    {
        ...
        "panels": [
            ...
            "B Model performance across validation designs",
            "C Calibration",
            "D Decision curve",
        ],
    },
...
def panel_label_at(ax: plt.Axes, label: str, x: float = -0.08, y: float = 1.08) -> None:
    ax.text(
        x,
        y,
        label,
...
def binary_group_tests(df: pd.DataFrame, group_col: str, features: list[str], positive_label: object = 1.0) -> pd.DataFrame:
    ...
    for feature in features:
        ...
        pos = pd.to_numeric(sub.loc[sub[group_col].astype(str).isin({str(positive_label), "1", "1.0", "True"}), feature], errors="coerce").dropna()
        neg = pd.to_numeric(sub.loc[~sub[group_col].astype(str).isin({str(positive_label), "1", "1.0", "True"}), feature], errors="coerce").dropna()
        if len(pos) < 2 or len(neg) < 2:
            u, p = np.nan, np.nan
        else:
...
def dot_matrix(
    ...
    *,
    ...
    ax.set_xlabel("")
    ax.set_ylabel("")
    cbar = ax.figure.colorbar(sc, ax=ax, cax=cbar_ax, fraction=cbar_fraction, pad=cbar_pad, shrink=cbar_shrink)
...
def plot_main_single_cell_response_landscape() -> None:
    ...
    comp_plot = comp[comp["state_category"].isin(key_states)].copy()
    ...
    if not network.empty:
        ...
        for xi, yi, row in zip(x, y, network.itertuples(index=False)):
            ...
            ax5.text(xi * 1.08, yi * 1.08, row.state_category, ha=ha, va="center", fontsize=6)
...
def plot_main_ai_reliability_sctime_modeling() -> None:
    ...
    axes["A"].text(
        0.02,
        0.98,
        f"Highlighted: scTIME-AI >= {FIGURE3A_RESEARCH_VALUE_THRESHOLD:.2f}",
    ...
    cbar.set_label("scTIME-AI score")
    ...
    axes["B"].set_title("scTIME-AI reliability across validation designs")
    axes["B"].set_xlabel("")
    ...
    sns.lineplot(data=dca, x="threshold", y="net_benefit_model", ax=axes["F"], label="scTIME-AI", color=PALETTE["positive"], linewidth=1.6)
    ...
    lollipop(axes["H"], coef_plot["label"], coef_plot["coefficient"], "Interpretable scTIME-AI coefficients", "ElasticNet coefficient")
    ...
    axes["I"].set_xlabel("Labeled bulk patients ordered by response and scTIME-AI score")
    axes["I"].set_ylabel("")
...
def plot_main_cxcl10_ifn_antigen_axis() -> None:
    ...
    lr_cxcl["response"] = response_label(lr_cxcl["benefit"], "MPR/pCR", "non-MPR/NE")
    sns.boxplot(data=lr_cxcl, x="response", y="interaction_score", hue="response", fliersize=0, palette=response_palette("non-MPR/NE", "MPR/pCR"), legend=False, ax=axes["C"])
...
def plot_main_myeloid_tgf_hypoxia_exclusion_axis() -> None:
    ...
    if "Myeloid/macrophage" in deconv.columns:
        sns.regplot(data=deconv, x="Myeloid/macrophage", y="SPP1_macrophage", scatter_kws={"s": 24, "alpha": 0.55}, line_kws={"color": PALETTE["dark"]}, ax=axes["B"])
        sub = deconv[["Myeloid/macrophage", "SPP1_macrophage"]].dropna()
        rho, p = stats.spearmanr(sub["Myeloid/macrophage"], sub["SPP1_macrophage"]) if len(sub) >= 4 else (np.nan, np.nan)
        axes["B"].text(0.05, 0.95, f"rho={rho:.2f}\nP={p:.2g}", transform=axes["B"].transAxes, va="top")
    axes["B"].set_title("Myeloid deconvolution moderately aligns with SPP1 score")
    ...
    dot_matrix(axes["C"], deconv_plot, "x", "module", "rho", "p_value", "Deconvolution-module correlation matrix", "Spearman rho")
    panel_label(axes["C"], "C")

    tcga_plot = tcga.dropna(subset=["immune_exclusion_index", "scTIME_AI_score"])
    hb = axes["D"].hexbin(
        tcga_plot["immune_exclusion_index"],
        tcga_plot["scTIME_AI_score"],
        gridsize=32,
        mincnt=1,
        cmap=SEQUENTIAL_CMAP,
        linewidths=0,
    )
    fig.colorbar(hb, ax=axes["D"], fraction=0.046, pad=0.03, label="Samples")
    rho, p = stats.spearmanr(tcga_plot["immune_exclusion_index"], tcga_plot["scTIME_AI_score"]) if len(tcga_plot) >= 4 else (np.nan, np.nan)
    axes["D"].text(0.05, 0.95, f"rho={rho:.2f}\nP={p:.2g}", transform=axes["D"].transAxes, va="top")
    axes["D"].set_title("TCGA exclusion-index density vs scTIME-AI")
    axes["D"].set_xlabel("Exclusion index")
    axes["D"].set_ylabel("scTIME-AI score")
    panel_label(axes["D"], "D")

    cptac_plot = cptac[cptac["stage_group"].isin(["Early", "Advanced"])].copy()
    cptac_plot["cohort_omics"] = cptac_plot["cohort"] + "\n" + cptac_plot["omics"]
    ...
    dot_matrix(axes["F"], tcga_corr_plot, "program", "target", "rho", "p_value", "TCGA exclusion co-program structure", "Spearman rho")
...
def plot_main_cd274_tcr_immune_ecology() -> None:
    ...
    labeled["combined_group"] = labeled["cd274_group"] + " / " + labeled["sctime_group"]
    group_resp = (
        labeled.groupby("combined_group", observed=True)
        .agg(n=("sample", "size"), responses=("benefit", "sum"), benefit_rate=("benefit", "mean"), mean_score=("scTIME_AI_score", "mean"), mean_cd274=("CD274_expr", "mean"))
        .reset_index()
    )
    group_resp["ci_low"], group_resp["ci_high"] = wilson_interval(group_resp["responses"], group_resp["n"])
    ...
    labeled["sctime_median_group"] = np.where(labeled["scTIME_AI_score"] >= labeled.groupby("dataset", observed=True)["scTIME_AI_score"].transform("median"), "scTIME-AI high", "scTIME-AI low")
    strat_rows = []
    for strategy, group_col in [
        ("scTIME-AI quartile", "sctime_confidence_group"),
        ("scTIME-AI median", "sctime_median_group"),
        ("CD274 median", "cd274_group"),
    ]:
            ...
            strat_rows.append(
                {
                    ...
                    "group": group,
                    "n": len(sub),
                    "responses": int(sub["benefit"].sum()),
                    "benefit_rate": float(sub["benefit"].mean()),
                    "mean_sctime_ai": float(sub["scTIME_AI_score"].mean()),
                    "mean_cd274": float(sub["CD274_expr"].mean()),
                }
            )
    stratified_resp = pd.DataFrame(strat_rows)
    stratified_resp["ci_low"], stratified_resp["ci_high"] = wilson_interval(stratified_resp["responses"], stratified_resp["n"])
    ...
    for feature in diff_features:
        ...
        rows.append(
            {
                "feature": feature,
                "high_score_mean": a.mean(),
                "cd274_high_score_low_mean": b.mean(),
                "difference": a.mean() - b.mean(),
                "p_value": pval,
                "n_high_score": len(a),
                "n_cd274_high_score_low": len(b),
            }
        )
    ...
    axes["A"].axvline(labeled["CD274_expr"].median(), color=PALETTE["neutral"], linestyle="--", linewidth=0.8)
    axes["A"].axhline(labeled["scTIME_AI_score"].median(), color=PALETTE["neutral"], linestyle="--", linewidth=0.8)
    rho, p = stats.spearmanr(labeled["CD274_expr"], labeled["scTIME_AI_score"])
    axes["A"].text(0.05, 0.95, f"rho={rho:.2f}\nP={p:.2g}", transform=axes["A"].transAxes, va="top")
    axes["A"].set_title("CD274 and scTIME-AI co-state in ICI cohorts")
    axes["A"].set_xlabel("CD274 expression/module")
    axes["A"].set_ylabel("scTIME-AI score")
    axes["A"].legend(frameon=False, fontsize=6)
    panel_label(axes["A"], "A")

    order = [
        "scTIME-AI bottom quartile",
        "scTIME-AI intermediate",
        "scTIME-AI top quartile",
        "CD274 low",
        "CD274 high",
    ]
    ...
    point_colors = [PALETTE["negative"], PALETTE["neutral"], PALETTE["positive"], PALETTE["neutral"], PALETTE["neutral"]][: len(resp_plot)]
    ...
    axes["B"].set_title("High-confidence scTIME-AI enrichment vs CD274")
    ...
    lollipop(axes["C"], contrast_plot["label"], contrast_plot["difference"], "CD274-high/scTIME-AI-low feature contrast", "High scTIME-AI minus CD274-high/scTIME-AI-low")
    panel_label(axes["C"], "C")
...
def plot_main_external_validation_cross_omics() -> None:
    ...
    axes["A"].set_title("GSE135222 PFS validation")
    ...
    axes["B"].set_xlabel("Hazard ratio per scTIME-AI score")
    axes["B"].set_ylabel("")
    panel_label(axes["B"], "B")
    ...
    axes["C"].set_title("TCGA-LUAD/LUSC scTIME-AI distributions")
    axes["C"].set_xlabel("")
    axes["C"].set_ylabel("scTIME-AI score")
    panel_label(axes["C"], "C")
    ...
    axes["D"].set_xlabel("")
    axes["D"].set_ylabel("scTIME-AI score")
    axes["D"].tick_params(axis="x", rotation=20)
...
def plot_supplementary_audit_model_details() -> None:
    ...
    axes["B"].set_title("Model performance across validation designs")
    ...
    sns.lineplot(data=dca, x="threshold", y="net_benefit_model", ax=axes["D"], label="scTIME-AI", color=PALETTE["positive"])
...
def plot_supplementary_translation_lr_details() -> None:
    ...
    axes["C"].set_ylabel("")
    panel_label_at(axes["C"], "C", x=-0.08, y=1.04)
    ...
    cax_d.tick_params(labelsize=6)
    cax_d.yaxis.label.set_size(7)
    cax_d.yaxis.labelpad = 2
    panel_label_at(axes["D"], "D", x=-0.08, y=1.12)
    ...
    cax_e.tick_params(labelsize=7)
    cax_e.yaxis.label.set_size(8)
    cax_e.yaxis.labelpad = 3
    axes["E"].set_title("Reference signature separability")
    axes["E"].set_xlabel("")
    axes["E"].set_ylabel("")
    panel_label_at(axes["E"], "E", x=-0.13, y=1.12)
    ...
    e_pos = axes["E"].get_position()
    axes["E"].set_position([e_pos.x0 + 0.025, e_pos.y0, e_pos.width, e_pos.height])
...
def write_figure_plan() -> None:
    ...
    with (DESIGN / "publication_figure_plan.json").open("w", encoding="utf-8") as handle:
        json.dump(plan, handle, ensure_ascii=False, indent=2)
    lines = ["# Publication Figure Design\n", "Main-text figures are organized as a framework figure plus six generated data figures that follow the immunotherapy-response storyline.\n"]
    lines.extend(
        [
            "\n## Recommended Main-Text Strategy\n",
            "- Figure 1 is a manually drawn study-framework schematic; the script does not generate it.\n",
            "- Generated main-text figures are numbered Figure 2 to Figure 7: single-cell discovery, scTIME-AI reliability, CXCL10/IFN axis, exclusion biology, CD274/TCR interpretation, and external cross-omics validation.\n",
    ...
    (DESIGN / "publication_figure_plan.md").write_text("".join(lines), encoding="utf-8")

    strategy = """# Figure Strategy for Frontiers in Immunology

## Main-Text Priority

1. Figure 1: Manually drawn study framework.
2. Figure 2: Single-cell response landscape.
3. Figure 3: scTIME-AI reliability, validation and interpretability.
4. Figure 4: CXCL10-CXCR3 / IFN-antigen evidence chain.
5. Figure 5: Myeloid/TGF-beta/hypoxia exclusion biology.
6. Figure 6: CD274 complementarity and TCR immune ecology.
7. Figure 7: External survival and cross-omics validation.
...
- The AI figure emphasizes final ElasticNet scTIME-AI reliability rather than visually promoting weaker exploratory algorithms.
...
"""
    (DESIGN / "frontiers_figure_strategy.md").write_text(strategy, encoding="utf-8")
