...
projection, response modelling, exploratory survival association, single-cell annotation
...
def zscore_from_reference(
    ...
    """Standardize target rows using means/SDs estimated from reference rows.

    The previous implementation standardized the complete cohort before CV,
    allowing held-out rows to influence their own preprocessing.  This helper
    keeps the cohort-specific scaling convention while estimating parameters
    only from the training portion of each split.  A target group absent from
    the reference falls back to pooled reference statistics, which is the
    conservative choice for cross-cohort projection.
    """
    ref = reference.copy()
    out = target.copy()
    pooled_stats: dict[str, tuple[float, float]] = {}
    group_stats: dict[tuple[object, str], tuple[float, float]] = {}
    for feature in features:
        values = pd.to_numeric(ref[feature], errors="coerce")
        pooled_stats[feature] = (float(values.mean()), float(values.std(ddof=0)))
        if group_col in ref.columns:
            for group, idx in ref.groupby(group_col, dropna=False).groups.items():
                group_values = pd.to_numeric(ref.loc[idx, feature], errors="coerce")
                group_stats[(group, feature)] = (float(group_values.mean()), float(group_values.std(ddof=0)))
    for feature in features:
        zname = f"z_{feature}"
        transformed = []
        for row_idx, row in out.iterrows():
            key = (row.get(group_col), feature)
            mean, sd = group_stats.get(key, pooled_stats[feature])
            ...
            if pd.isna(value):
                ...
                transformed.append((float(value) - mean) / sd)
        out[zname] = transformed
    return ref, out
...
def nested_candidate_selection_predictions(
    ...
    """Create outer-held-out predictions with fold-local preprocessing."""
    feature_cols = [f"z_{feature}" for feature in features]
    outer = StratifiedKFold(n_splits=n_outer_splits, shuffle=True, random_state=1)
    nested_probability = np.full(len(y), np.nan, dtype=float)
    nested_fold = np.full(len(y), np.nan)
    selection_rows: list[dict[str, object]] = []
    for outer_fold, (train_idx, test_idx) in enumerate(outer.split(labeled, y), start=1):
        train_outer = labeled.iloc[train_idx].copy()
        test_outer = labeled.iloc[test_idx].copy()
        train_y = y[train_idx]
        inner_splits = max(2, min(4, int(pd.Series(train_y).value_counts().min())))
        inner = StratifiedKFold(n_splits=inner_splits, shuffle=True, random_state=100 + outer_fold)
        candidate_scores: dict[str, float] = {}
        for name, candidate in models.items():
            inner_probability = np.full(len(train_outer), np.nan, dtype=float)
            for inner_train_idx, inner_test_idx in inner.split(train_outer, train_y):
                inner_train = train_outer.iloc[inner_train_idx]
                inner_test = train_outer.iloc[inner_test_idx]
                _, inner_train_scaled = zscore_from_reference(inner_train, inner_train, features)
                _, inner_test_scaled = zscore_from_reference(inner_train, inner_test, features)
                fitted = clone(candidate).fit(inner_train_scaled[feature_cols], train_y[inner_train_idx])
                inner_probability[inner_test_idx] = fitted.predict_proba(inner_test_scaled[feature_cols])[:, 1]
            candidate_scores[name] = safe_auc(train_y, inner_probability)
        selected = sorted(candidate_scores.items(), key=lambda item: (-float(item[1]), item[0]))[0][0]
        _, train_scaled = zscore_from_reference(train_outer, train_outer, features)
        _, test_scaled = zscore_from_reference(train_outer, test_outer, features)
        fitted = clone(models[selected]).fit(train_scaled[feature_cols], train_y)
        nested_probability[test_idx] = fitted.predict_proba(test_scaled[feature_cols])[:, 1]
        nested_fold[test_idx] = outer_fold
        for candidate_name, inner_auc in sorted(candidate_scores.items()):
            selection_rows.append(
                {
                    "outer_fold": outer_fold,
                    "candidate_model": candidate_name,
                    "inner_roc_auc": inner_auc,
                    "selected_model": selected,
                    "outer_test_n": len(test_idx),
                    "outer_test_positives": int(y[test_idx].sum()),
                }
            )
    if np.isnan(nested_probability).any():
        raise RuntimeError("Nested candidate-selection predictions are incomplete")
    nested = labeled[["dataset", "sample", "endpoint", "endpoint_text", "benefit"]].copy()
    nested["model"] = "Nested_selected_model"
    nested["prediction"] = nested_probability
    nested["evaluation"] = "nested_5x4fold_candidate_selection"
    nested["prediction_provenance"] = "nested_outer_holdout"
    nested["fold"] = nested_fold
    return nested, pd.DataFrame(selection_rows)
...
def evaluate_response_models(bulk_scores: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, Pipeline, list[str]]:
    ...
    # Full-cohort scores are retained for the final refit and external
    # projections.  OOF evaluation below recomputes these transforms from
    # training rows inside each fold.
    ...
    cv_splits = list(cv.split(labeled, y))
    models = make_models()
    perf_rows = []
    prediction_frames = []
    ...
    for name, model in models.items():
        ...
        try:
            prob = np.full(len(labeled), np.nan, dtype=float)
            for train_idx, test_idx in cv_splits:
                _, train_scaled = zscore_from_reference(labeled.iloc[train_idx], labeled.iloc[train_idx], features)
                _, test_scaled = zscore_from_reference(labeled.iloc[train_idx], labeled.iloc[test_idx], features)
                fitted = clone(model).fit(train_scaled[feature_cols], y[train_idx])
                prob[test_idx] = fitted.predict_proba(test_scaled[feature_cols])[:, 1]
            perf_rows.append(
                {
    ...
    for train_ds, test_ds in [("GSE207422_bulk", "GSE126044"), ("GSE126044", "GSE207422_bulk")]:
        train = labeled[labeled["dataset"] == train_ds].copy()
        test = labeled[labeled["dataset"] == test_ds].copy()
        if train["benefit"].nunique() < 2 or test["benefit"].nunique() < 2:
            continue
        for name, model in models.items():
            try:
                # For a cross-cohort test, fit scaling on the training cohort
                # and apply those fixed parameters to the held-out cohort.
                _, train_scaled = zscore_from_reference(train, train, features)
                _, test_scaled = zscore_from_reference(train, test, features)
                fitted = clone(model).fit(train_scaled[feature_cols], train["benefit"].astype(int))
                prob = fitted.predict_proba(test_scaled[feature_cols])[:, 1]
                perf_rows.append(
                    {
    ...
    nested_pred, nested_selection = nested_candidate_selection_predictions(labeled, features, y, models, n_splits)
    prediction_frames.append(nested_pred)
    nested_auc = safe_auc(y, nested_pred["prediction"])
    nested_pr = safe_average_precision(y, nested_pred["prediction"])
    nested_brier = float(brier_score_loss(y, nested_pred["prediction"]))
    perf_rows.append(
        {
            "evaluation": "nested_5x4fold_candidate_selection",
            "model": "Nested_selected_model",
            "train_dataset": "GSE126044+GSE207422_bulk",
            "test_dataset": "GSE126044+GSE207422_bulk",
            "n": len(y),
            "positives": int(y.sum()),
    ...
    nested_selection.to_csv(TABLES / "nested_model_selection.tsv", sep="\t", index=False)
    pd.DataFrame(
        [{"evaluation": "nested_5x4fold_candidate_selection", "roc_auc": nested_auc, "pr_auc": nested_pr, "brier": nested_brier, "n": len(y), "positives": int(y.sum())}]
    ).to_csv(TABLES / "nested_selection_sensitivity.tsv", sep="\t", index=False)
...
def save_model_details(model: Pipeline, feature_cols: list[str], labeled: pd.DataFrame, final_prob: np.ndarray) -> None:
    ...
    metadata = {
        "model_name": "scTIME ElasticNet logistic regression",
...
def plot_model_outputs(
    ...
    if not coef.empty:
        ...
        ax.set_ylabel("")
        ax.set_title("scTIME feature weights")
        fig.tight_layout()
...
def plot_km(gse: pd.DataFrame) -> None:
    ...
    for label, group_value, color in [("High scTIME", 1, "#2f6f9f"), ("Low scTIME", 0, "#b4473d")]:
    ...
    ax.set_title("GSE135222 exploratory PFS association by scTIME score")
...
def single_cell_gse243013() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    response_text = sample_clin["pathological_response"].astype(str).str.strip().str.lower()
    is_response = response_text.isin({"mpr", "pcr"})
    is_non_response = response_text.str.fullmatch(r"non[- ]?mpr", case=False, na=False)
    pd1_text = sample_clin["anti_PD1_therapy"].astype(str).str.strip().str.lower()
    has_pd1 = ~pd1_text.isin({"no", "nan", "none", "", "unknown", "na"})
    sample_clin["analysis_included"] = (has_pd1 & (is_response | is_non_response)).astype(bool)
    sample_clin["benefit"] = np.where(sample_clin["analysis_included"], is_response.astype(float), np.nan)
...
def plot_gse243013_sc(comp: pd.DataFrame, diff: pd.DataFrame) -> None:
    ...
    if not plot.empty:
        plot = plot.dropna(subset=["benefit"])
        plot["benefit_group"] = np.where(plot["benefit"] == 1, "MPR/pCR", "non-MPR")
...
def gse207422_sc_pseudobulk() -> pd.DataFrame:
    ...
    response_text = scores["pathologic_response"].astype(str).str.strip().str.lower()
    scores["benefit"] = np.where(response_text.isin({"mpr", "pcr"}), 1.0, np.where(response_text.eq("nmpr"), 0.0, np.nan))
    ...
    if not plot.empty:
        plot = plot.dropna(subset=["benefit"])
        plot["benefit_group"] = np.where(plot["benefit"] == 1, "MPR/pCR", "NMPR")
...
def gse207422_sample_metadata() -> pd.DataFrame:
    ...
    response_text = meta["pathologic_response"].astype(str).str.strip().str.lower()
    meta["benefit"] = np.where(response_text.isin({"mpr", "pcr"}), 1.0, np.where(response_text.eq("nmpr"), 0.0, np.nan))
    return meta
...
def tcga_clinical(project: str) -> pd.DataFrame:
    ...
    for patient, sub in clinical.groupby("cases.submitter_id", dropna=True):
        ...
        # A death record without a death time is not a valid OS observation;
        # do not silently substitute last follow-up for an event time.
        os_time = death if os_event == 1 else follow
        rows.append(
            {
                "project": project,
                "patient": patient,
                ...
                "os_time_valid": bool(pd.notna(os_time) and float(os_time) > 0),
            }
        )
    out = pd.DataFrame(rows)
...
def tcga_secondary_validation(
    ...
    for project in ["TCGA-LUAD", "TCGA-LUSC"]:
        ...
        sample_barcodes = pd.Series(expr.columns.astype(str), index=expr.columns)
        # Xena STAR-count matrices contain primary tumour (01), recurrent
        # tumour (02) and normal adjacent (11) aliquots.  Restrict survival
        # projection to primary tumour aliquots and aggregate duplicate
        # aliquots to one patient-level score.
        sample_type = sample_barcodes.str.split("-").str[-1].str.slice(0, 2)
        primary_columns = sample_type.index[sample_type.eq("01")].tolist()
        expr = expr.loc[:, primary_columns]
            ...
            rows.append(
                {
                    ...
                    "n_expression_patients": len(expr_patients),
        ...
        # Patient-level mean across any remaining primary-tumour aliquots.
        score_cols = [c for c in scores.columns if c not in {"sample", "patient"}]
        scores = scores.groupby(["dataset", "patient"], as_index=False)[score_cols].mean(numeric_only=True)
        scores["sample"] = scores["patient"]
        scores["sample_type"] = "Primary Tumor"
        ...
        surv = merged[merged["os_time_valid"].eq(True)].dropna(subset=["os_time", "os_event", "scTIME_AI_score"]).copy()
...
def plot_tcga_survival(scores: pd.DataFrame) -> None:
    ...
    for ax, (project, sub) in zip(axes.ravel(), plot.groupby("project", observed=True)):
        ...
        for label, value, color in [("High scTIME", 1, "#2f6f9f"), ("Low scTIME", 0, "#b4473d")]:
...
def cptac_secondary_validation(final_model: Pipeline, feature_cols: list[str]) -> pd.DataFrame:
    ...
    if not plot.empty:
        ...
        ax.legend(handles[:2], labels[:2], frameon=False, title="")
        ax.set_xlabel("")
        ax.set_ylabel("Projected scTIME score")
...
def projection_summary_by_histology() -> None:
    ...
    def histology_group(x: object) -> str:
        text = str(x).lower()
        if "squamous" in text and "adeno" in text:
            return "Mixed adenocarcinoma+squamous"
        if "squamous" in text:
            return "Pure squamous"
        if "adeno" in text:
    ...
    ax.set_xlabel("")
    ax.set_ylabel("scTIME projected score")
...
def write_report(
    ...
    if not survival.empty:
        report.append("- GSE135222 exploratory PFS association:")
    ...
    report.append("- `results/models`: serialized final scTIME model and metadata.")
